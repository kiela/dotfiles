...
`.zshrc`, links this repo's zsh setup and links each script from `bin/`
into `~/bin`, which `zshrc` puts on `PATH` (several git aliases depend
...
Because those files execute code, a directory has to be allowed first:
    ...
    dirrc-show [dir]      # print what would be sourced
    dirrc-trust [dir]     # allow (stored in ~/.dirrc_trusted)
    dirrc-untrust [dir]   # revoke

All three default to the current directory. `$HOME` and `~/.aliases`
are always trusted.
